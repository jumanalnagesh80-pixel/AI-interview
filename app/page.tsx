...
function TrustBar() {
  const stats = [
    { k: "120k+", v: "interviews scored" },
    { k: "4.9 / 5", v: "user rating" },
    { k: "8+", v: "company simulators" },
    { k: "<200ms", v: "real-time feedback" },
  ];
  return (
      ...
      <div className="grid grid-cols-2 gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:grid-cols-4">
        {stats.map((s) => (
          <div key={s.v} className="text-center">
            <div className="text-2xl font-semibold tracking-tight text-gradient sm:text-3xl">{s.k}</div>
            <div className="text-xs uppercase tracking-widest text-white/45">{s.v}</div>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="mx-auto mt-20 max-w-7xl px-4 sm:px-6">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {features.map((f) => (
          <div key={f.title} className="group relative card transition hover:border-white/20 hover:bg-white/[0.05]">
            {f.tag && (
